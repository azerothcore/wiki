...
Esta tabla contiene los textos que se usan para el gossip. Hay que investigar más sobre esta tabla.
...
Este es el ID de la entrada de texto.

### text0_0 to text7_0

Este es el texto localizado que se muestra si el NPC es masculino.

### text0_1 to text7_1

Este es el texto localizado que se muestra si el NPC es femenino.

### BroadcastTextID0 to BroadcastTextID7

El valor del campo \`broadcast\_text\`.\`ID\` para el \`MaleText\` en la tabla \`broadcast\_text\`.

### lang0 to lang7

El idioma del texto en el juego. Consulta [esta página](languages) para los IDs de idioma disponibles.
Si se pone a 0, entonces no requiere conocer ningún idioma específico.

### Probability0 to Probability7

La probabilidad de que el NPC diga text0\_0 o text0\_1 según el género del NPC.

### em0_0-5 to em7_0-5

El ID del emote que el NPC debe hacer al mostrar el texto. Los emotes se reproducen uno tras otro en orden. Para ver una lista de emotes, consulta Emotes.DBC

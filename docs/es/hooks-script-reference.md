...
## Catálogo completo de ScriptMgr
...
Todos los nombres y firmas de hooks de esta página se copian de `ScriptMgr.h` y deben tratarse como la superficie canónica del API, incluso cuando un identificador del core conserva una grafía heredada como `AnticheatSetCanFlybyServer` o `AnticheatSetJumpingbyOpcode`.
...
### `ConditionScript` (1 firma)

```cpp
bool OnConditionCheck(Condition* condition, ConditionSourceInfo& sourceInfo);
```

### `VehicleScript` (6 firmas)
...
### `DynamicObjectScript` (1 firma)

```cpp
void OnDynamicObjectUpdate(DynamicObject* dynobj, uint32 diff);
```

### `TransportScript` (5 firmas)
...
### `AchievementCriteriaScript` (1 firma)
...
### `MovementHandlerScript` (1 firma)

```cpp
void OnPlayerMove(Player* player, MovementInfo movementInfo, uint32 opcode);
```

### `AllCreatureScript` (3 firmas)
...
### `AllGameObjectScript` (1 firma)

```cpp
void OnGameObjectSaveToDB(GameObject* go);
```

### `AllMapScript` (2 firmas)

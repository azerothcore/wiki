...
- `CanAddMember`, `OnAddMember`, `OnGetPoints`: hooks de `ArenaScript` específicamente para validación de membresía de `ArenaTeam`, manejo posterior a añadir un miembro y la fórmula de puntos del equipo basada en el rating del miembro.
- `OnGetArenaPoints`, `OnGetSlotByType`: hooks de `ArenaTeamScript` para resolución de slots y valores de puntos de arena expuestos por el sistema de arena teams.

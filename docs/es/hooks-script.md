...
- `CanAddMember`, `OnAddMember`, `OnGetPoints`: hooks de `ArenaScript` específicamente para validación de membresía de `ArenaTeam`, manejo posterior a añadir un miembro y cálculo de puntos de miembro/equipo mediante `ArenaScript::OnGetPoints`.
- `OnGetArenaPoints`, `OnGetSlotByType`: hooks de `ArenaTeamScript` para resolución de slots y valores de puntos de arena mediante `ArenaTeamScript::OnGetArenaPoints`.

...
**spell\_scripts:** contiene los scripts que pueden activarse mediante hechizos con el efecto SPELL\_EFFECT\_SCRIPT\_EFFECT (77) o SPELL\_EFFECT\_DUMMY(3).

**event\_scripts:** contiene los scripts que se activan cada vez que se activa un evento, ya sea por un objeto o como el spell effect SPELL\_EFFECT\_SEND\_EVENT (61).

**waypoint\_scripts:** contiene los scripts usados en la tabla [waypoint\_data](waypoint_data). Ver también [Waypoints-Information](waypoints-information) para información general sobre los waypoints.

NOTA: una entrada de esta tabla puede tener más de una fila, ya que un script puede hacer más de una acción. Además, cada acción que realice el script puede tener un delay independiente asociado. En ese caso, el core activará la acción apropiada tras el delay correcto.
...
Para **spell\_scripts**, es el ID del hechizo. Ver [Spell.dbc](spell)

Para **event\_scripts**, es el ID del evento. Actualmente no existe una lista completa de eventos. En cualquier caso, los IDs de evento se toman directamente de los datos WDB del gameobject o de los datos del spell effect. Si tanto un gameobject como un hechizo activan el mismo evento, los IDs coincidirán.

Para **waypoint\_scripts**, es el ID de la [action](waypoint_data#action).

### effIndex

El índice del efecto del hechizo al que se va a aplicar este script.

### delay

Delay en segundos antes de que se active este paso actual del script. 0 = instantáneo.

### command

El tipo de acción que realiza el script tras haber pasado [delay](#delay) segundos. El valor de este campo determina qué otros campos también deben establecerse. Se pueden usar los siguientes comandos:
...
Dependiendo del comando usado, el significado y uso de los siguientes campos varía.
...
- dataint: referencia a [broadcast\_text.id](broadcast_text)
...
Existe solo para 'waypoint_scripts' y actúa allí como clave primaria; se establece automáticamente usando el [comando GM](gm-commands) 'wp event add'.

...
If value is [-Client Build](http://www.azerothcore.org/wiki/realmlist#gamebuild) then it was parsed with WDB files from that specific [client build](http://www.azerothcore.org/wiki/realmlist#gamebuild) and manually edited later for some special necessity.

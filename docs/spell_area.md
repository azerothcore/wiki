...
The entry of the quest which the player must have in the state defined by **quest\_start\_status**. See [quest\_template.id](http://www.azerothcore.org/wiki/quest_template#id).

### quest\_end

The entry of the quest which the player must not have in the state defined by **quest\_end\_status**. See [quest\_template.id](http://www.azerothcore.org/wiki/quest_template#id). Setting both **quest\_start** and **quest\_end** to the same value is useless.
...
```sql
INSERT INTO spell\_area (spell, area, quest\_start, quest\_end, autocast, quest\_start\_status, quest\_end\_status) VALUES 

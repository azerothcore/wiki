...
Contains extra definitions like linking quests, dependencies and requirements for the quests defined in the [quest\_template](http://www.azerothcore.org/wiki/quest_template) table to become available to the player.
...
<td><a href="http://www.azerothcore.org/wiki/quest_template#id">Unique ID linked to quest_template.ID</a></td>
</tr>
<tr class="even">
...
Unique quest ID, matching the same quest ID in [quest\_template.ID](http://www.azerothcore.org/wiki/quest_template#id)
...
See the [examples section](http://www.azerothcore.org/wiki/quest_template#examples-dealing-with-quests) for examples.
...
Note: All quests that use an ExclusiveGroup must also have entries in [pool\_template[(http://www.azerothcore.org/wiki/pool_template) and [pool\_quest](http://www.azerothcore.org/wiki/pool_quest) in order for the core to choose one randomly. See the [examples section](http://www.azerothcore.org/wiki/quest_template#examples-dealing-with-quests) for examples.

### **RewardMailTemplateID**

If the quest gives as a reward an item from a possible list of items, the ID here corresponds to the proper loot template in [quest\_mail\_loot\_template](http://www.azerothcore.org/wiki/loot_template). According to the rules in that loot template, items "looted" will be sent by mail at the completion of the quest.

### **RewardMailDelay**

How many seconds to wait until the mail is sent to the character that turned in a quest rewarding items from a loot template defined in [RewardMailTemplateId](http://www.azerothcore.org/wiki/quest_template#rewardmailtemplateid)
...
-    2: Makes the quest only completable by some external event (an entry in [areatrigger\_involvedrelation](http://www.azerothcore.org/wiki/areatrigger_involvedrelation), spell effect quest complete or an entry in [spell\_scripts](http://www.azerothcore.org/wiki/scripts) with command 7 as some examples)
